...
contract VaultV3UpgradeableV2 is OwnableUpgradeable, PausableUpgradeable, ReentrancyGuard, UUPSUpgradeable {
    ...
    // FIXED: Calculate minNovisOut based on actual swap amount (after treasury fee)
    function _tryAutoBuyAndBurn() internal {
        ...
        // FIX: Calculate expected output based on post-fee swap amount
    ...
    function _executeBuyAndBurn(uint256 usdcAmount, uint256 minNovisOut) internal {
        ...
        if (treasuryFee > 0 && treasury != address(0)) { usdc.safeTransfer(treasury, treasuryFee); }
